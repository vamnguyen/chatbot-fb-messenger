  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      request(
        ...
        (err, res, body) => {
          console.log("🚀 ~ file: chatBotService.js:35~ body:", body);
          if (!err) {
...
let handleGetStarted = (sender_psid) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // send text message
      let response1 = {
        text: `Welcome ${username} to VAM Nguyen Restaurant! 🎉🎊😍`,
      };
...
let guideToUseBot = (sender_psid) => {
  return new Promise(async (resolve, reject) => {
    try {
      const username = await getUserName(sender_psid);
      // send text message
      let response1 = {
        text: `Hello ${username}! I'm chatbot is generated by VAM. For more of my features watch the video below 🙊😘🥰`,
      };

      // send video template guide to use bot
